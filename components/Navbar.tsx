...
export default function Navbar({ user }: { user: any }) {
  return (
    ...
    <nav className="fixed top-0 left-0 w-full bg-white shadow-lg py-2.5 border-b border-gray-200 z-50 cursor-pointer dark:bg-slate-950 dark:border-slate-900">
      <div className="container mx-auto flex justify-between items-center px-5 lg:px-1">
        {/* Left Logo */}
        <Link href={"/"} className="flex items-center gap-2">
          {/* <Image src={"/logo.png"} alt="logo" width={50} height={50} priority/> */}
          <span className="text-2xl font-bold text-slate-800 uppercase dark:text-white">LemonSqueezy</span>
        </Link>
